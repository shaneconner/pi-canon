...
- The shared Codex and Claude Code hook cannot inspect that projection. It uses an explicit compaction cycle instead: session start, context clear, and compaction clear its seen set; resume does not. None of those events surfaces an article. A later asset touch does.
