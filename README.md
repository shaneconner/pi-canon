...
- **`threshold`** is how good a match has to be before it may spend a line, on the 0 to 1 scale every retriever normalizes to. Default `0`, which admits anything the query touched at all. Raise it to trade recall for precision, and precision is the side that matters here: an unsolicited line that is usually noise teaches the agent to skip the next one, and suggestion fatigue costs more than the tokens do. A benchmark session at `0` was handed 28 ranked lines and opened 5 of them; the scores of the opened and the ignored overlap but separate, median 0.25 against 0.19, which is the precondition a cutoff needs to be worth having. Where to put it is not yet measured on a corpus with anything worth finding in its residue, so the default changes nothing and the number is yours. Out of range throws at registration rather than comparing false against every score and silently turning retrieval off.
