...
| `search` | `query` | Ranks articles and journal entries against the words in one pass, ten results, each carrying what scopes it: an article its address and capsule, a journal entry its instant and subjects. Articles are guaranteed half the window, because measured on real stores the journal entries about an event otherwise crowd out the article carrying its current truth; a short side cedes its slots. Says how many matches the cap dropped. The one action that reaches the journal's content. |
