...
Reading an article through `pi_canon` withdraws the line staged for it before the message goes out, so pull preempts push. Reading the asset file itself does not, because reading a file is not reading what is known about it, and the capsule may hold exactly the constraint the file does not contain. After the agent settles, articles touched but not updated draw one reminder naming them, once per batch, re-armed by the next touch. The reverse case answers at the write instead: recording an article about an asset this session touched draws one line in the write's own result asking that the asset be checked against the rule as recorded, because the session that writes a rule down is the one most likely to have just broken it, and a settle reminder rides a turn that a session ending at the write never sees. Once per article per session.
